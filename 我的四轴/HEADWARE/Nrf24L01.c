...
uint8_t NRF_Read_Buf(uint8_t reg, uint8_t *pBuf, uint8_t uchars)
{
	...
	for(i=0; i<uchars; i++)
	{
		pBuf[i] = SPI2_ReadWriteByte(0xff); /* 读取返回数据 */ 	
	}
	NRF_DISENABLE;						/* 禁止该器件 */
    return 	status;
}
...
/////////////////////////////////////////////////////////
	if(model==1)				//RX
	{
		...
		NRF_Write_Reg(NRF_WRITE_REG + CONFIG, 0x0f);   		 // IRQ收发完成中断开启,16位CRC,主接收,上电
	}
	else if(model==2)		//TX
	{
		NRF_Write_Reg(NRF_WRITE_REG+RX_PW_P0,RX_PLOAD_WIDTH);								//选择通道0的有效数据宽度 
...
{  
	...
	GPIO_EXTILineConfig(GPIO_PortSourceGPIOA,GPIO_PinSource0);  //GPIOE.2 中断线以及中断初始化配置，下降沿触发     
