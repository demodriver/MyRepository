...
{
	if (USART_GetFlagStatus(USART1, USART_FLAG_ORE) != RESET) //中断错误异常检查
    {
        USART_ReceiveData(USART1);//抛弃
    }
	//发送中断
	...
	if(USART1->SR & (1<<5))//if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)    
	{
		...
		static u8 _data_len = 0,_data_cnt = 0;//数据长度
		...
		else if(RxState==3&&com_data<50)//数据长度
		{
			RxState = 4;
			RxBuffer[3]=com_data;
			_data_len = com_data;
		...
		else if(RxState==5)//校验字
		{
			RxState = 0;
			RxBuffer[4+_data_cnt]=com_data;
			//Data_Receive_Anl(RxBuffer,_data_cnt+5);//数据分析
		}
		else
			RxState = 0;
	}	
...
void Uart1_Put_Buf(unsigned char *DataToSend , u8 data_num)
{
	...
	if(!(USART1->CR1 & USART_CR1_TXEIE))//发送数据缓冲区空
		USART_ITConfig(USART1, USART_IT_TXE, ENABLE); 
}
